...
<script>$(document).ready(function(c) {
	$('.alert-close').on('click', function(c){
		window.location="buyDel.php?id="+$('.alert-close').attr("id")+"&url=admin_users.php";
	});	  
});
</script>
</head>
			    ...
			    <?php
					...
					if(!isset($username)){
						...
						if(session_id()!=$sessin_id){
							...
							echo "<a href='Tlogout.php?pre=admin_users.php'> 注销</a>";
							$cdt=true;
						}
					}
				?>
...
<div class="checkout">
  ...
  <table>
    <tbody>
    	<form method="post" action="Tadmin_users.php">
    		<tr><td>ID</td><td>密码</td><td>二级密码</td><td>姓</td><td>名</td><td>E-mail</td></tr>
    <?php
        ...
        for($i=0;$i<count($data['users']);++$i){
        	echo '<tr><td><input type="text" name="id'.$i.'" value="'.$data['users'][$i]["id"].'"></td>
        			  <td><input type="text" name="pw'.$i.'" value="'.$data['users'][$i]["passwd"].'"></td>
        			  <td><input type="text" name="pw2'.$i.'" value="'.$data['users'][$i]["passwd2"].'"></td>
        			  <td><input type="text" name="x'.$i.'" value="'.$data['users'][$i]["xing"].'"></td>
        			  <td><input type="text" name="m'.$i.'" value="'.$data['users'][$i]["ming"].'"></td>
        			  <td><input type="text" name="em'.$i.'" value="'.$data['users'][$i]["email"].'"></td></tr>';
        	}
	  ?>
    </tbody>
  </table>
  <br>
   <!--<input type="submit" value="添加" style="margin:-10px 0 20px 400px;width:120px" onclick="">-->
