...
<script>$(document).ready(function(c) {
	$('.alert-close').on('click', function(c){
		window.location="buyDel.php?id="+$('.alert-close').attr("id")+"&url=checkout.php";
	});	  
});
</script>
</head>
			    ...
			    <?php
					...
					if(!isset($username)){
						...
						if(session_id()!=$sessin_id){
							...
							echo "<a href='Tlogout.php?pre=checkout.php'> 注销</a>";
							$cdt=true;
						}
					}
				?>
...
<div class="checkout">
	 <div class="container">
		...
		<?php
		    $s=0;
		    $arr=$_SESSION["mycar"];
		    if (empty($a)) echo '<h1 class="title1">购物车空空如也。</h1>
		<p class="cart">您的购物车里没有物品。<br>点击<a href="index.php">这里</a>返回。</p>';
			else{
		  	foreach($arr as $a){
		  		$s=$s+$a["num"]*$a["money"];
		?>
		<div class="cart_box">
			<div class="message">
				<?php echo '<div class="alert-close" id="'.$a["pid"].'"></div>' ?> 
				<div class="list_imgg" width="80px"> <?php echo '<img src="'.$a["url"].'" class="img-responsive" width="80px" alt=""/>' ?> </div>
				<div class="list_desc"><h4><a href="#"><?php echo $a["name"]?></a></h4><?php echo $a["num"]?> x<span class="actual">￥<?php echo $a["money"]?></span></div>
			    <div class="clearfix"></div>
		    </div>
		</div>
		<?php
		 	...
		 	echo '<div class="total">
	            	<div class="total_left">总计 : </div>
	            	<div class="total_right">￥'.$s.'</div>
	            	<div class="clearfix"> </div>
	            </div>
                <div class="login_buttons">
					<div class="login_button"><a href="checkout.php">结账</a></div>
					<div class="clearfix"></div>
				</div>';
		 }
		?>
